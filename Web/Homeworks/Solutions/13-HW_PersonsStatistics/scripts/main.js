...
var statisticsSectionElement = document.getElementById('statistics');
var thStatisticsElement = document.getElementById('trHeadStatistics');
var tbodyStatisticsElement = document.getElementById('names-table-statistics');
var listPopularNamesElement = document.getElementById('list-statistics');
var tableStatistics = new Table(['name', 'occurrences'],
    thStatisticsElement, tbodyStatisticsElement);
/**************************************************************/
//functions
function getStatisticsView() {
    mainElement.classList.add(HIDDEN);
    statisticsSectionElement.classList.remove(HIDDEN);
    getStatistics();
}
function onBackStatistics() {
    mainElement.classList.remove(HIDDEN);
    statisticsSectionElement.classList.add(HIDDEN);
}
...
function addItemToList(listElement, person) {
...
function displayOldestPersons () {
    ...
    persons.forEach(function (person) {
        if (person.birthYear === minBirthYear ) {
            addItemToList(oldestPersonsElement, person)
        }
    })
}
...
function getStatistics() {
    var nameStatistics = getNameStatistics();//gets array
    // of {name,occurrences}
    nameStatistics.sort(function (no1, no2) {
        return no1.name <= no2.name ? -1 : 1
    });
    drawTableStatistics(nameStatistics);
    drawMostPopularNames(nameStatistics);

}
function getNameStatistics() {
    var statisticsObj = {}
    var personNames = persons.map(function(p) {
        return p.name;
    });
    ...
    var allNames = Object.keys(statisticsObj);
    var nameOccurrences = allNames.map(function(n) {
        return {name: n, occurrences: statisticsObj[n]};
    })
    return nameOccurrences;
}
function drawTableStatistics(nameStatistics) {
    tableStatistics.clear();
    nameStatistics.forEach(function(ns) {
        tableStatistics.addRow(ns);
    })
}
function drawMostPopularNames(nameStatistics) {
    // var maxOccurrences = nameStatistics.reduce
    // (function(maxOc, current) {
    //     return current.occurrences > maxOc ? current.occurrences
    //         : maxOc
    // }, 0); //at each iteration we have maxOC as the current max value
    // and current as a current object from nameStatistics
    var maxOccurrences = nameStatistics.reduce
    (function(maxOc, current) {
        return current.occurrences > maxOc ? current.occurrences
            : maxOc
    ...
    // nameStatistics.forEach(function(ns) {
    //     if (ns.occurrences > maxOccurrences) {
    //         maxOccurrences = ns.occurrences;
    //     }
    // })
    removeChildren(listPopularNamesElement);
    nameStatistics.forEach(function (ns) {
        if (ns.occurrences == maxOccurrences) {
            addItemToList(listPopularNamesElement, ns.name);
        }
    })
}
/********************************************/
