...
LogRepository logRepository;
	...
	public Flux<LogType> getMostEncounteredExceptionTypes(int nExceptions) {
		...
		return logRepository.getMostEncountedEcxeptionTypes(nExceptions).map(e->e.logType);
	}
	...
	public Mono<List<String>> getMostEncounteredArtifacts(int nArtifacts) {
		...
		return logRepository.getMostEncounteredArtifacts(nArtifacts).map(e->e.artifact).collectList();
	}
