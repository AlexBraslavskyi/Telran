...
WebTestClient testClient;
	private void getRequest(String uriStr, HttpStatus status) {
		testClient.get().uri(uriStr)
		.exchange().expectStatus().isEqualTo(status);
	}

	private void postRequest(String uriStr, Object invalidObject, HttpStatus status) {
		testClient.post().uri(uriStr).contentType(MediaType.APPLICATION_JSON).bodyValue(invalidObject)
		.exchange().expectStatus().isEqualTo(status);
	}
	private void putRequest(String uriStr, Object invalidObject, HttpStatus status) {
	...
	void closeBug() {
		...
		putRequest(BUGS_CLOSE, closeData, HttpStatus.OK);
	}
	@Test
	@Order(7)
	...
	void invalidAddProgrammer() {
		//email not correct
		postRequest(BUGS_PROGRAMMERS, new Programmer(1, "Moshe", "kuku"), HttpStatus.BAD_REQUEST);
		//id < 1
		postRequest(BUGS_PROGRAMMERS, new Programmer(0, "Moshe", MOSHE_EMAIL), HttpStatus.BAD_REQUEST);
		//name - null
		postRequest(BUGS_PROGRAMMERS, new Programmer(1, null, "kuku"), HttpStatus.BAD_REQUEST);
	}
	@Test
	void invalidDuplicateAddProgrammers() {
		// programmer already exist
		postRequest(BUGS_PROGRAMMERS, new ProgrammerDto(PROGRAMMER_ID_VALUE_MOSHE,"Moshe", MOSHE_EMAIL), HttpStatus.CONFLICT);
	}
	@Test
	void invalidAddArtifact() {
		//Empty artifactID
		postRequest(BUGS_ARTIFACTS, new ArtifactDto("", PROGRAMMER_ID_VALUE_MOSHE), HttpStatus.BAD_REQUEST);
		//not found programmerID
		postRequest(BUGS_ARTIFACTS, new ArtifactDto(ARTIFACT_ID, 555), HttpStatus.NOT_FOUND);
		//programmerId < 1
		postRequest(BUGS_ARTIFACTS, new ArtifactDto(ARTIFACT_ID, -1), HttpStatus.BAD_REQUEST);
	}

	@Test
	void invalidOpenBug() {
		//seriousness null
		postRequest(BUGS_OPEN, new BugDto(null, "desc", LocalDate.now()), HttpStatus.BAD_REQUEST);
		//description null
		postRequest(BUGS_OPEN, new BugDto(Seriousness.BLOCKING, null, LocalDate.now()), HttpStatus.BAD_REQUEST);
	}

	@Test
	void invalidOpenAssignBug() {
		//programmer id negative
		postRequest(BUGS_OPEN_ASSIGN, new BugAssignDto(Seriousness.BLOCKING,
				DESCRIPTION, DATE_OPEN, -20), HttpStatus.BAD_REQUEST);
		//not found programmer
		postRequest(BUGS_OPEN_ASSIGN, new BugAssignDto(Seriousness.BLOCKING,
				DESCRIPTION, DATE_OPEN, 555), HttpStatus.NOT_FOUND);
	}
	...
	void invalidAssignBug() {
		//bugId<1
		putRequest(BUGS_ASSIGN, new AssignBugData(0, PROGRAMMER_ID_VALUE_MOSHE, DESCRIPTION), HttpStatus.BAD_REQUEST);
		//programmer not found
		putRequest(BUGS_ASSIGN, new AssignBugData(1, 555, DESCRIPTION), HttpStatus.NOT_FOUND);
		//bug not found
		putRequest(BUGS_ASSIGN, new AssignBugData(222, PROGRAMMER_ID_VALUE_MOSHE, DESCRIPTION), HttpStatus.NOT_FOUND);
		//bug already assigned
		putRequest(BUGS_ASSIGN, new AssignBugData(1, PROGRAMMER_ID_VALUE_MOSHE, DESCRIPTION), HttpStatus.CONFLICT);
	}
	@Test
	void invalidCloseBug() {
		//bugId<1
		putRequest(BUGS_CLOSE, new CloseBugData(0, null, null), HttpStatus.BAD_REQUEST);
		//bug not found
		putRequest(BUGS_CLOSE, new CloseBugData(222, null, null), HttpStatus.NOT_FOUND);
	}
	@Test
	void invalidUnclosedBugsDuration() {
		//nDays < 1
		getRequest(BUGS_UNCLOSED + "?" + N_DAYS + "=" + "-1", HttpStatus.BAD_REQUEST);
	
	}
	@Test
	void invalidProgrammerIdInGetBugsProgrammer() {
		//programmerId < 1
		getRequest(BUGS_PROGRAMMERS + "?" + PROGRAMMER_ID + "=" + "-1", HttpStatus.BAD_REQUEST);
	}
	@Test
	void invalidN_ProgrammersInProgrammersMostBug() {
		//nProgrammers < 1
		getRequest(BUGS_PROGRAMMERS_MOST + "?" + N_PROGRAMMERS + "=" + "-1", HttpStatus.BAD_REQUEST);
		
	}
	@Test
	void invalidN_ProgrammersInProgrammersLeastBug() {
		//nProgrammers < 1
		getRequest(BUGS_PROGRAMMERS_LEAST + "?" + N_PROGRAMMERS + "=" + "-1", HttpStatus.BAD_REQUEST);
		
	}
	@Test
	void invalidN_TypesInSeriousnessMost() {
		//nTypes < 1
		getRequest(BUGS_SERIOUSNESS_MOST + "?" + N_TYPES + "=" + "-1", HttpStatus.BAD_REQUEST);
